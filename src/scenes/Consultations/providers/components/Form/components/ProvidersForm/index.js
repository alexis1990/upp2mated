...
const isSupplierExisting = (contacts, index) => contacts[index].supplier.length > 0;
		...
		<Row className="show-grid">
		    {fields.map((field, index) =>
				<Col xs={12} md={12} lg={12} key={index} className="fields">
					<Row className="show-grid">
				      	<Col sm={2} md={2} lg={4}>
					      	<Field type="select" withButton withGlyph="plus" onClick={() => isModalVisible(true)} onSearch={onSearch}   options={suppliers} placeholder="Chercher fournisseur par nom" withoutLabel name={`consultationSupplierList[${index}].supplier`} component={asyncTypeAhead}>Chercher fournisseur par nom</Field>
				      	</Col>
						<Col sm={2} md={2} lg={3}>
