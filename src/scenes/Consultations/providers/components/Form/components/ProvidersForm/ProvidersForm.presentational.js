...
const ProvidersForm = ({ handleSubmit, fields, listProviders, addProvidersField, removeProvidersField, isModalVisible }) => (console.log('FIEEEEE', fields),
		...
		<Row className="show-grid">
		    {fields.map((field, index) =>
				<Col xs={12} md={12} lg={12} key={index} className="fields">
					<Row className="show-grid">
				      	<Col sm={2} md={2} lg={2}>
					      	<Field type="select" withButton withGlyph="plus" onClick={() => isModalVisible(true)} options={listProviders} placeholder="Chercher fournisseur par nom" withoutLabel name={`providersReducer[${index}].provider`} component={selectTypeahead}>Chercher fournisseur par nom</Field>
				      	</Col>
				      	<Col sm={2} md={2} lg={2}>
