...
const Modal = ({ component, isVisible, isModalVisible, nameModal, activeNameModal }) => (
  <div>
    <Grid onClick={() => isModalVisible(false)} className={`modal_background ${isVisible && nameModal === activeNameModal ? 'visible' : 'hidden'}`} />
    <Row className={`modal_container ${isVisible && nameModal === activeNameModal ? 'visible' : 'hidden'}`}>
      <Button className="pull-right" onClick={() => isModalVisible(false)} bsClass="action-button-transparent small-text" type="button">
